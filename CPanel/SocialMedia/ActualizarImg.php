...
$consulta = "SELECT imagen from socialmedia Where id='1'";
$i=0;
$idc="";
$Imagen="";
...
if (move_uploaded_file($nombre_archivo_temporal, $destino))
    {
        $sqlo="update socialmedia set imagen='$nombre_archivo_original' WHERE id='1'";
            ...
            if($resultado1 =='1')
            {
                if (!unlink($file))
                {
                    echo'<script type="text/javascript">
							alert("Ha ocurrido un error grave, contacte el administrador lo antes posible");
							window.location.href="../Ofertas.php";
						</script>';
                }
                else
                {
                    header("location:../SocialMedia.php?accion=updateC");
                }
            }
            else
            {
